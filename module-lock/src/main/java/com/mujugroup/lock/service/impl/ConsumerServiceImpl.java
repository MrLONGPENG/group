        ...
        if (infoTo != null) {
            ...
            if (new Date().getTime() - lockInfo.getLastRefresh().getTime() < FailTask.TIME_SPAN) {
                ...
                if (lockInfo.getLockStatus().equals(4)) {
                    LockFail lockFailOpen = lockFailService.getFailInfoByDid(infoTo.getDid(), Integer.parseInt(LockFail.FAIL_TYPE_SWITCH), Integer.parseInt(LockFail.FE_SW_OPEN));
                    if (lockFailOpen == null) {
                        ...
                        lockFailService.getModel(lockFailOpen, Integer.parseInt(infoTo.getAid()), Integer.parseInt(infoTo.getHid())
                                , Integer.parseInt(infoTo.getOid()), Long.parseLong(infoTo.getDid()), Integer.parseInt(LockFail.FAIL_TYPE_SWITCH)
                                , Integer.parseInt(LockFail.FE_SW_OPEN), lockInfo.getLastRefresh(), Long.parseLong(infoTo.getBid()));
            ...
            if (lockInfo.getLockStatus().equals(5)) {
                LockFail lockFailClose = lockFailService.getFailInfoByDid(infoTo.getDid(), Integer.parseInt(LockFail.FAIL_TYPE_SWITCH), Integer.parseInt(LockFail.FE_SW_CLOSE));
                if (lockFailClose == null) {
                    ...
                    lockFailService.getModel(lockFailClose, Integer.parseInt(infoTo.getAid()), Integer.parseInt(infoTo.getHid())
                            , Integer.parseInt(infoTo.getOid()), Long.parseLong(infoTo.getDid()), Integer.parseInt(LockFail.FAIL_TYPE_SWITCH)
                            , Integer.parseInt(LockFail.FE_SW_CLOSE), lockInfo.getLastRefresh(), Long.parseLong(infoTo.getBid()));
        ...
        if (infoTo != null) {
            ...
            if (new Date().getTime() - lockInfo.getLastRefresh().getTime() < FailTask.TIME_SPAN) {
                ...
                if (lockInfo.getCsq().equals(-1)) {
                    LockFail lockFailOffline = lockFailService.getFailInfoByDid(infoTo.getDid(), Integer.parseInt(LockFail.FAIL_TYPE_SIGNAL), Integer.parseInt(LockFail.FE_SG_OFFLINE));
                    if (lockFailOffline == null) {
                        ...
                        lockFailService.getModel(lockFailOffline, Integer.parseInt(infoTo.getAid()), Integer.parseInt(infoTo.getHid())
                                , Integer.parseInt(infoTo.getOid()), Long.parseLong(infoTo.getDid()), Integer.parseInt(LockFail.FAIL_TYPE_SIGNAL)
                                , Integer.parseInt(LockFail.FE_SG_OFFLINE), lockInfo.getLastRefresh(), Long.parseLong(infoTo.getBid()));
